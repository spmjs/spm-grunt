...
function patchTask(grunt) {
  ...
  grunt.loadGlobalTasks = function(name) {
    ...
    var nodePaths = NODE_PATH.split(os.platform().indexOf('win') !== -1 ? ';' : ':');
    var rootdir, pkgfile, pkg, taskdir;
    for ( var i = 0, size = nodePaths.length; i < size; i++ ) {
        rootdir = path.join(nodePaths[i], name);
        if ( !grunt.file.exists(rootdir) ) continue;

        pkgfile = path.join(rootdir, 'package.json');
        pkg = grunt.file.exists(pkgfile) ? grunt.file.readJSON(pkgfile): {keywords: []};

        taskdir = path.join(rootdir, 'tasks');
        ...
        if (pkg.keywords && pkg.keywords.indexOf('gruntcollection') !== -1) {
          ...
          Object.keys(pkg.dependencies).forEach(function(depName) {
            ...
            if (grunt.file.exists(filepath)) {
              ...
              grunt.loadGlobalTasks(path.relative(nodePaths[i], filepath));
            }
          });
